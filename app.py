...
def save_conversation(conversation: str) -> str:
    with open(
        f"./GEMINI_CHATS/{SESSION_TIMESTAMP}_{MODEL}_Chat.md", "w+",
...
while program_on:
    ...
    console.print(Panel(user_input, title="User",
                  border_style="blue", highlight=True))
    ...
    try:
        ...
        spinner_thread = threading.Thread(
            target=spinner, args=(lambda: stop_spinner,))
        ...
        console.print(
            Panel(markdown, title="Gemini response", border_style="green"))
        print("\n" * 3)
...
# Use color for the exit message.
